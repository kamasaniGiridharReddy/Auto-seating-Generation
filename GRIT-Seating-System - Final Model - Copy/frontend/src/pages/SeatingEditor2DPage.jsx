...
function SortableSeat({ seat, isConflict, onDragStart }) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export default function SeatingEditor2DPage() {
  ...
  // Generate bench layout
  const benches = []
  for (let row = 0; row < classroomGrid.rows; row++) {
    for (let col = 0; col < classroomGrid.columns; col++) {
      const benchIndex = row * classroomGrid.columns + col
      const benchNumber = benchIndex + 1
      
      const benchSeats = []
      for (let i = 0; i < classroomGrid.studentsPerBench; i++) {
        const seatNo = benchIndex * classroomGrid.studentsPerBench + i + 1
        const seat = roomSeating.find(s => (s.seatNo || s.seatingNo) === seatNo)
        benchSeats.push(seat || { seatNo, status: 'Empty' })
      }
                    ...
                    >
                      {bench.seats.map((seat) => (
                        <SortableSeat
                          key={seat.seatNo}
                          seat={seat}
                          isConflict={conflicts.has(seat.seatNo)}
                        />
                      ))}
                    </div>
                  </div>
