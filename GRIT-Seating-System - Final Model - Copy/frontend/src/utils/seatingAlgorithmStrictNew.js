...
function seatStudentsStrict(students, positions, grid, rows, cols, studentsPerBench, orientation, roomName, startSeatNumber = 1) {
  ...
  console.log(`[Strict Seating] Seating ${students.length} students in ${roomName}`)
  ...
  if (remainingStudents.length > 0) {
    console.log('[Strict Seating] FINAL PASS: Seat remaining students')
...
function allocateSeatingSingleAttempt(students, classrooms, attemptIndex = 0) {
  ...
  console.log(`[Strict Seating] Room distribution:`, roomDistribution.map(r => `${r.roomName}: ${r.studentsCount}`).join(', '))
